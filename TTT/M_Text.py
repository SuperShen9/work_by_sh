...
for row1 in range(2,sheethy.get_highest_row()+1):
...
for k in range(2,hang):
    if sheet['B'+str(k)].value[-2:] in hydata.keys() or sheet['C'+str(k)].value[-2:] in hydata.keys():
        if sheet1['B'+str(k)].value[:2]<>sheet1['C'+str(k)].value[:2]:
            sheet['D' + str(k)].value = '行业3'
        else:
            sheet['D' + str(k)].value = '行业甄别'
    elif sheet['B'+str(k)].value==sheet['C'+str(k)].value:
        sheet['D' + str(k)].value = '1'
    elif sheet['B'+str(k)].value[:2]<>sheet['C'+str(k)].value[:2]:
        sheet['D'+str(k)].value='3'
    else:
        sheet['D' + str(k)].value = 'DD'
sheet['A1'] = 'ID'
